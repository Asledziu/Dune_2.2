...
namespace RTSEngine
{
    ...
    public class BuildingPlacer : MonoBehaviour
    {
        ...
        //Building can be playced near other faction buildings
        [SerializeField]
        private bool placeNearOwnBuildings = false;
        ...
        private bool runTestCollision = false;
        ...
        private void FixedUpdate()
        {
            ...
            if (IsActive && building.FactionID == GameManager.PlayerFactionID && building.Placed == false) // && NewPos == true) //& only if the building hasn't been placed yet
            {
                CheckBuildingPos();
            }
        }
        ...
        public void CheckBuildingPos()
        {
...
#endif
            if (NewPos) // check only if bulding is on new position
            {
                ...
                if (!IsBuildingInCenterRange() || !IsBuildingOnMap() || !IsBuildingNearResource() || !IsBuldingNearOtherBuilding())
                {
                    ...
                    testCollision = false;
                    runTestCollision = false;
                }
                else
                {
                    runTestCollision = true;
                }
                NewPos = false;
            }
            ...
            if (runTestCollision)
            {
                //check allways in order to manage current collisions
                if (testCollision == false) //if we haven't started the collision test, start it
                {
        ...
        //method to check if building is near other building (if it need to be)
        public bool IsBuldingNearOtherBuilding()
        {
            if (placeNearOwnBuildings)
            {
                foreach (Collider col in Physics.OverlapBox(building.BoundaryCollider.bounds.center, building.BoundaryCollider.bounds.extents * 1.5f))
                {
                    if (col != building.BoundaryCollider)
                    {
                        Building bul = col.transform.GetComponent<Building>();
                        if (bul != null)
                        {
                            if (building.FactionID == bul.FactionID)
                            {
                                return true;
                            }
                        }
